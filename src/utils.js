...
*/
export const extractTorqueData = ({data, currentProfile, currentDirection}) => {
    const filteredData =  data.filter(({Direction,  Profile} ) => 
        ...
        return {
            AverageTorque,LastTorque, 
            x: Position, //relabel Position with 'x' for chartJS
        };
    })
    const labels = Array //gives out x-axis labels
    .from( new Set( 
        filteredData.map(({x}) => Number(x))
    ) )
